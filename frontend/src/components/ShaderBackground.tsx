...
//
// lolprofiles.gg: the recipe colours below are the site palette (warm-black
// through champagne-gold, tokens `--bg` → `--gold`), so the silk reads as a
// slow gold current over the near-black ground rather than the stock blue.
// It is decorative only — `aria-hidden`, and the hero lays a scrim over it so
// the wordmark, lede and search field keep full contrast.
...
// Recipe colours are the site palette, warm-black → champagne-gold:
//   --bg #0b0908 · #241c12 · --gold-dim #8c6d2f · --gold #e8b54a · --gold-bright #f5c968
// A wide gold band in the middle of the ramp keeps visible gold current over the
// near-black ground; the last entries repeat gold-bright for the array length
// the shader expects.
const UNIFORMS = {
  colors: [
    [0.043137254901960784, 0.03529411764705882, 0.03137254901960784],
    [0.1411764705882353, 0.10980392156862745, 0.07058823529411765],
    [0.5490196078431373, 0.42745098039215684, 0.1843137254901961],
    [0.9098039215686274, 0.7098039215686275, 0.2901960784313726],
    [0.9607843137254902, 0.788235294117647, 0.40784313725490196],
    [0.9607843137254902, 0.788235294117647, 0.40784313725490196],
    [0.9607843137254902, 0.788235294117647, 0.40784313725490196],
    [0.9607843137254902, 0.788235294117647, 0.40784313725490196],
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // `getContext` returns null when WebGL is unavailable and, under jsdom,
    // throws outright ("not implemented") — both mean "no canvas to drive".
    // Kept `const` so its non-null narrowing survives into the cleanup closure.
    ...
    // Arrow rather than a hoisted `function` so it inherits the non-null
    // narrowing of `canvas` and `gl` from the guards above.
