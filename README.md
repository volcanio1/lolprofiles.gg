...
| `CRAWLER_ENABLED` | No | `false` | `1` / `true` / `yes` turns on the [champion build pipeline](#champion-build-pipeline). Also needs `MONGODB_URI`. Off means the endpoint returns its empty-state and the champion page shows "Not enough games recorded". |
| `CRAWL_BUDGET_FRACTION` | No | `0.25` | Share of `CRAWL_RPS` the crawler self-limits to, clamped to `(0, 1]`. |
| `CRAWL_RPS` | No | `0.8` | Conservative estimate of the app's sustained Riot requests/second budget (a dev key is ~0.8). Raise it with a production key. |
| `CRAWL_INTERVAL_MS` | No | `300000` | Crawl cycle cadence. |
| `CRAWL_SEEDS_PER_CYCLE` | No | `25` | Seed players crawled per cycle. |
| `CRAWL_MATCHES_PER_SEED` | No | `20` | Recent ranked match ids fetched per seed. |
...
**Cold-start.** Every number comes from **this site's own aggregation of crawled ranked matches** — Riot publishes no aggregate endpoint. The [champion build pipeline](#champion-build-pipeline) that fills the store is **off by default** (`CRAWLER_ENABLED`); until it has run for a while the endpoint returns a `200` with `popular` / `highestWinRate` `null` and empty `meta` option lists, and the page renders "Not enough games recorded" everywhere — the same cold-start behaviour `/api/players/suggest` shipped with. The endpoint holds no per-player data, so `POST /api/privacy/delete` has nothing to clear from it.
...
| `champion_build_aggregates` | By the offline crawler (see [Champion build pipeline](#champion-build-pipeline)) — `$inc` upserts, keyed `champion\|role\|bucket\|region\|patch` | Per-cell `games` / `wins` + capped frequency sub-maps (item paths, and per item path: skill orders / rune pages / spell pairs / starting items) — **aggregates only, never a raw match** | [`GET /api/champions/:championKey/build-stats`](#get-apichampionschampionkeybuild-stats) |
| `champion_build_totals` | By the crawler, once per crawled match per rank bucket | `{ _id: bucket\|region\|patch, matches }` — the pick-rate denominator | the same endpoint |
| `crawl_seeds` | By the crawler's weekly ladder walk (overwritten, not appended) | `{ _id: puuid, tier, platform, refreshedAt }` — ladder-public PUUIDs to crawl from | the crawler |
| `crawl_processed` | By the crawler after a match is aggregated | `{ _id: matchId, processedAt }` — a TTL-bounded seen-set so a match is counted exactly once | the crawler |

The first four collections' writes are **fire-and-forget** — issued as unawaited side effects of a lookup, never on its critical path. A slow, unreachable, or erroring database degrades to "the graph is a little younger, one name is missing from autocomplete, a dropdown pick does a normal live lookup, and a match is re-fetched", never to a slow or failed lookup. `POST /api/privacy/delete` clears `rank_snapshots`, `looked_up_players`, `profile_reports` and `match_details` for a PUUID alongside the cache (a `match_details` document the PUUID appears in is evicted whole, not redacted). It does **not** touch the `champion_build_*` / `crawl_*` collections: the aggregates hold no personal data, and the `crawl_*` PUUIDs are ladder-public identifiers the crawler discovered from League-V4, not looked-up-user data — they age out on their own TTL / next seed refresh.
...
`champion_build_aggregates` is capped at its source, not just watched: every frequency sub-map holds at most `MAX_FREQ_KEYS` (40) entries, and docs for patches older than the newest two are pruned each crawl cycle. At ~170 champions × 6 roles × 4 buckets × 2 patches ≈ 8,000 documents — most sparse (a few KB), the `{ALL,ALL}` and main-role docs for meta champions near a ~150 KB cap — the store lands around **150–250 MB** steady state. Without the cap and prune the worst case (every doc full) would not fit M0, which is why both are load-bearing. `crawl_processed` carries a **120-day TTL** on `processedAt`; at real crawl volume it is the collection to watch second.
...
### Champion build pipeline

The `champion_build_*` collections are filled by **one background worker** (`CrawlWorker`, a sibling of the Clash Tournament Refresher), **off by default** — it runs only when `CRAWLER_ENABLED` is truthy *and* `MONGODB_URI` is set. Each cycle (default every 5 min) it:

1. prunes old-patch aggregate docs (opportunistic — a failure is harmless);
2. refreshes the seed pool if the freshest `crawl_seeds` entry is over a week old, by walking the League-V4 Challenger/GM/Master lists and paged Emerald/Diamond entries for a fixed set of platforms (`euw1`, `na1`, `kr` — `region` is the label `world`);
3. takes the next `CRAWL_SEEDS_PER_CYCLE` seed players (a cursor in `crawl_state` walks the pool), fetches each one's recent **queue-420** match ids, and for each id not already in `crawl_processed`: fetches the match detail + timeline, rebuilds each human participant's core item path / skill order / rune page / spells from the timeline, and folds the result into the `(champion, role, bucket, patch)` aggregates via `$inc` — then records the match as processed so it is counted exactly once.

**Rate budget.** The crawler and live lookups draw from the **same** `RateLimitManager`. A `CrawlGate` token bucket keeps crawler demand below `CRAWL_RPS × CRAWL_BUDGET_FRACTION` requests/second (default `0.8 × 0.25`), and any `rate_limited` result abandons the cycle until the next tick. This makes contention rare — but with one shared budget and no request priority, "live traffic always has headroom" is a *statistical* property, not a hard guarantee; the mitigation is a conservative `CRAWL_RPS` and a low fraction. Nothing on a request path ever calls the crawler.

**Dev-key reality.** A dev key grants ~100 requests / 2 min. A full seed+crawl pass at that rate is **1–3 weeks** and competes directly with every live lookup. The crawler is correct and safe to run, but meaningful per-patch freshness needs a production key. Until the store has data, [`GET /api/champions/:championKey/build-stats`](#get-apichampionschampionkeybuild-stats) returns its empty-state response and the champion page shows "Not enough games recorded" everywhere — the same cold-start behaviour `/api/players/suggest` shipped with.
...
- **The champion build pipeline is off by default and slow to fill on a dev key.** The [crawler](#champion-build-pipeline) that fills `champion_build_aggregates` is built and wired, but `CRAWLER_ENABLED` is unset by default and a full seed+crawl pass is 1–3 weeks on a dev key while sharing the live rate budget — so `/api/champions/:championKey/build-stats` and the `/champion/:key` page show "Not enough games recorded" until it has run. The "live traffic always has headroom" property of the `CrawlGate` is statistical, not guaranteed, given one shared `RateLimitManager` and no request priority. Multi-region crawling and finer rank granularity than the three coarse buckets are deferred; the completed-item set is pinned at one Data Dragon version while the crawler aggregates recent patches, so an item reworked component↔legendary between the pin and a crawled patch is briefly misclassified.
