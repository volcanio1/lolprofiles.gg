...
- Match-V5's participant record reports the **final inventory at game end**, not a purchase sequence. Requirement 3.8 requires the UI to say so. A real purchase timeline needs Match-V5's timeline endpoint, whose payloads are one to five megabytes per match and would force a cache-backing decision before it could ship — out of scope here.
