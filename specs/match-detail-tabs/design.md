...
**Scoreboard redesign (revised 2026-08-27, to the dpm.lol reference).** The 10-column `ScoreboardRow` was overflowing the detail panel. Redesigned to fit at page width with readable text:
- **Champion column** is the portrait (32 px), then the champion name with `Lv N` on a line under that name. The name is `ChampionIcon`'s own — that component renders portrait *and* display name inside one `.champion-icon-label` — so `GeneralTab` must not render a second one. To stack the level under the name rather than after the whole label, `.sb-champ-inner` is a 2-column grid and the label wrapper is `display: contents`, promoting the portrait and the name into it (portrait spans both rows, name row 1, level row 2).
- The **player name + `#tag`** is its own headerless column, set apart to the right, and wraps within that column.
- **Row heights are uniform, and that is load-bearing.** `height` on a `<tr>` is only a minimum, so a name long enough to wrap grew *that one row*; because the champion cell is `vertical-align: middle`, its portrait/name/level then re-centred in the taller row and the champion names stopped lining up down the column (the support row, in practice — its player name was the long one). The fix is that `.sb-name-line` always reserves exactly two lines (`min-height`/`max-height: 2.4em` with `align-content: center`), so every row is the same height whether or not any name actually wraps. A name too long for two lines is clipped, with the full `name#tag` on the cell's `title`.
- Spells and runes share **one unlabelled column** (two mini-columns of stacked 16 px icons).
- Damage/gold use `k`-notation (`formatCompactNumber`, one decimal under 100k); the damage cell carries a **bar scaled to the match's highest damage** (gold for the analyzed row, dimmed gold otherwise).
- Row height comes from `0.5rem` padding + the two-line identity; cell text is `0.82rem`.
- **Header and cell alignment match per column** — number columns (`.sb-num`) are right-aligned on both `th` and `td`, everything else left. `.scoreboard-table .sb-num` is used so the rule out-specifies the base `th, td` rule.
- The table no longer borrows `.data-table`; it carries its own `border-collapse`, header rule, and row dividers. `.table-scroll` remains only as a narrow-viewport fallback.

**Performance rating (added 2026-08-27).** Each row ends with a **0–100 rating** of how the player played, colour-coded: **red < 45**, **grey 45–74**, **gold + glow ≥ 75**. Computed by `frontend/src/domain/matchRating.ts` (pure, tested). Design agreed with the user:
- *Hybrid* — every stat (KDA, kill participation, CS/min, vision/min, damage share, gold share, objective last-hits) is scored against absolute, role-adjusted "poor / good / great" benchmarks; the weighted blend is then nudged (a sub-15-minute game compresses toward 50, since the numbers mean less).
- *Somewhat role-aware* — support and jungle weight kill participation, vision, and objectives much higher; the lanes weight CS, damage, and gold. Weights and benchmarks are per-`teamPosition` tables.
- *Outcome matters, lightly* — a win adds +3; a loss caps the score at 95 (still "great"-eligible, not perfect).
- This needed three new fields on `MatchParticipant` (`turretKills`, `dragonKills`, `baronKills`) — plain Match-V5 objective last-hits, always present — threaded through `toMatchParticipant` and the wire type. `durationSeconds` is threaded from `DetailPanel` into `GeneralTab` for the per-minute stats.
