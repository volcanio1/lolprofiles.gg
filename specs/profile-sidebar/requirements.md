...
2. THE System SHALL keep a new Rank_Snapshot only once the player has played about 3 more ranked games in that queue since the last one kept (or their tier/division changed), so repeated lookups between games do not distort the graph's spacing and the graph advances roughly per few games rather than per lookup. See `specs/database/` Requirement 2.2 for the exact rule.
...
5. THE System SHALL label the graph's horizontal axis neutrally (e.g. "recorded over time"), not "games played," since the System still only observes a player when someone looks them up — a Rank_Snapshot is one such observation, kept when enough ranked games have passed since the last (`specs/database/` Requirement 2.2), never a reading taken after every completed game.
